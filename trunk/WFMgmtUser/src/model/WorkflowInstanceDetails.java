...
public class WorkflowInstanceDetails {
	...
	public static WorkflowInstanceDetails selectone(int wfinstanceid) {
		...
		try {
			while (resultSet.next()) {
				...
				wfInstanceDetails.workflowInstance.getWfcategory()
						.setWfcategoryid(resultSet.getInt("wfcategoryid"));
				wfInstanceDetails.workflowInstance.getWfcategory()
						.setWfcategoryname(
	...
	public int createnewInstance() {
		int instanceID = 0;
		ResultSet resultSet = null;
		String query = "call create_workflow_instance("
				+ this.getWorkflowInstance().getOwner().getMemberid() + ","
				+ this.getWorkflowType().getWftypeid() + ","
				+ this.getWorkflowInstance().getWfcategory().getWfcategoryid()
				+ ");";
		...
		try {
			while (resultSet.next()) {
				instanceID = resultSet.getInt("wfinstanceid");
			}
		} catch (SQLException e) {
	...
	/*
	 * "INSERT INTO workflowinstance (MemberID, WFTypeID, CreationDateTime, CurrentState, WFItemID,WFCategoryID) "
	 * + " VALUES(" + this.getWorkflowInstance().getOwner().getMemberid() + ","
	 * + this.getWorkflowType().getWftypeid() + ", " + "sysdate()," +
	 * this.getWorkflowInstance().getAction().getActionid() + "," + "NULL," +
	 * this.getWorkflowInstance().getWfcategory().getWfcategoryid() + ");";
	 */
}
